    ...
    <div class="max-w-3xl mx-auto p-6 bg-white shadow-md rounded-lg mt-10">
        ...
        <form action="{{ route('expense.update', $expense->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="jenis" id="jenis" class="w-full border border-gray-300 rounded-lg p-2">
                    @foreach ($jenis_expense as $item)
                        <option value="{{ $item->id }}"
                            @if ($item->id == $expense->jenis_expense) selected @endif>
                            {{ $item->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="nama" id="nama" value="{{ $expense->nama ?? '' }}" class="w-full border border-gray-300 rounded-lg p-2" placeholder="Masukkan nama expense">
            </div>

            <!-- Jumlah -->
            <div>
                <label for="jumlah" class="block text-lg font-semibold mb-2">Jumlah</label>
                <input type="text" name="jumlah" id="jumlah" value="{{ $expense->jumlah ?? '' }}" class="w-full border border-gray-300 rounded-lg p-2" placeholder="Masukkan jumlah expense">
            </div>
